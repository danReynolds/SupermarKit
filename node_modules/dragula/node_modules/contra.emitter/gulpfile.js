...
gulp.task('build-shim', ['bump', 'test', 'clean'], function () {
  ...
  return gulp.src('./src/contra.emitter.shim.js')
    .pipe(header(extended, { pkg : pkg } ))
    .pipe(gulp.dest('./dist'))
    .pipe(rename('contra.emitter.shim.min.js'))
    .pipe(uglify())
...
gulp.task('build', ['build-shim'], function () {
  ...
  return gulp.src('./src/contra.emitter.js')
    .pipe(header(extended, { pkg : pkg } ))
    .pipe(gulp.dest('./dist'))
    .pipe(rename('contra.emitter.min.js'))
    .pipe(uglify())
    .pipe(header(succint, { pkg : pkg } ))
    .pipe(size())
    .pipe(gulp.dest('./dist'));

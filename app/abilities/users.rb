Canard::Abilities.for(:user) do
  ...
  cannot :update,                                   UserGroup,                  id: @user.user_group_ids
  can    :update,                                   UserGroup,                  owner_id: @user.id
  cannot :leave,                                    UserGroup,                  owner_id: @user.id
  ...
  can    :manage,                                   Item, Item.all do |item|
                                                      UserGroup.public.union(@user.user_groups)
                                                        .merge(item.user_groups).length.nonzero?
                                                    end

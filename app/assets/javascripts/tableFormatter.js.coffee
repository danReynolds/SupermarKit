@formatItems = (json) ->
  ...
  $.each json.data, (i, item) ->
    dataRow = [
      ...
      "<a href='#' data-dropdown='dropdown-#{item.id}' aria-controls='dropdown-#{item.id}' aria-expanded='false' class='dropdown'><i class='fa fa-pencil-square-o'></i></a><br> <ul id='dropdown-#{item.id}' data-dropdown-content class='tiny f-dropdown' aria-hidden='true'> <li><a href='/items/#{item.id}'>View</a></li><li><a class='remove' href='#'>Remove</a></li> </ul>",
    ]
